...
`onTelemetry` receives a `ToolTelemetryEvent` enum:

| Case | When |
| --- | --- |
| `.requested(call)` | The model requested this tool call |
| `.approved(call)` | `approval` returned `.execute` or `.replaceArguments` |
| `.rejected(call, reason:)` | `approval` or `onToolCall` returned `.reject` |
| `.started(call)` | The tool closure is about to run |
| `.succeeded(call, result)` | The tool closure returned successfully |
| `.failed(call, message:)` | The tool closure threw |
| `.skipped(call, reason:)` | A synthetic result was returned without executing |
| `.completed(stepIndex:)` | All tool calls for the step finished |

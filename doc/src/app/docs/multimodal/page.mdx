...
There is no dedicated `.audioURL` prompt constructor in the current package. For audio prompts, load the bytes into `Data` or pass an existing base64 string. Use [audio](/docs/audio) for dedicated transcription and speech APIs. Multimodal input is for model understanding; media APIs are for producing or transcribing media.

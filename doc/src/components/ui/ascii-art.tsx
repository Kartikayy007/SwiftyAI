...
type AsciiArtProps = {
  ...
  /** Drop transparent + near-black matte pixels (for logos with removed backgrounds) */
...
export const AsciiArt: React.FC<AsciiArtProps> = ({
      ...
      for (let y = 0; y < rows; y++) {
        ...
        for (let x = 0; x < cols; x++) {
          ...
          const isEmpty =
            ...
            (alphaNorm < 0.15 || (alphaNorm > 0.85 && luminance < 32));

          if (isEmpty) {
            row.push({ char: " ", r: 0, g: 0, b: 0, empty: true });
            continue;
          }

          const brightness = luminance / 255;
          const adjustedBrightness = brightness * Math.min(alphaNorm, 1);
  ...
  }, [src, resolution, effectiveCharset, objectFit, transparentBackground]);
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    // Only pre-draw static frame when animation is disabled; otherwise let animation effect handle it.
    if (shouldShowStatic || !animated) {
      drawCanvas(1);
    }
  }, [isLoaded, asciiData, drawCanvas, shouldShowStatic, animated]);
